...
3. **single vue app per editor iframe**, not per block. `EditorRoot.vue` mounts once, walks `[data-block-vue]` markers, teleports block components into them. shared reactivity context. avoids the "n parallel vue apps" anti-pattern.
