...
(function (global, doc, Translator) {
  ...
  var refreshTrashModal = function refreshTrashModal(event) {
    ...
    if (numberOfSubitems) {
      var message = Translator.trans(/*@Desc("Sending '%content%' and its %children_count% Content item(s) to Trash will also send the sub-items of this Location to Trash.")*/'trash_container.modal.message_main', {
        content: contentName,
        children_count: numberOfSubitems
      }, 'ibexa_content');
      modalBody.querySelector('.ibexa-modal__option-description').innerHTML = message;
    } else {
      var _message = Translator.trans(/*@Desc("Are you sure you want to send this Content item to Trash?")*/'trash.modal.message', {}, 'ibexa_content');
